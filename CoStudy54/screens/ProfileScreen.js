...
import { session, proStatus, myBookings, cancelBooking, myApplications, updateProfile, deleteBooking, listUsers } from "../api";
...
export default function ProfileScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const map = {};
      (Array.isArray(u) ? u : []).forEach((usr) => { map[usr.userId] = usr; });
      setUsersById(map);
  ...
  const logout = () => {
    Alert.alert("Log out?", "Are you sure you want to log out?", [
      ...
      { text: "Yes, Log Out", style: "destructive", onPress: () => {
        session.token = null; session.user = null; navigation.getParent()?.replace("Login");
      }}
    ]);
  };
  ...
  const canDelete = (status) => { const s = (status || "").toUpperCase(); return s === "COMPLETED" || s === "CANCELLED" || s === "CANCELED"; };

  const doDelete = (b) => {
    Alert.alert("Delete booking?", "This will remove the booking from your list. This cannot be undone.", [
      { text: "Cancel", style: "cancel" },
      { text: "Delete", style: "destructive", onPress: async () => {
        try { await deleteBooking(b.bookingId); await load(); }
        ...
        <BlurView intensity={28} tint="light" style={styles.profileInfoCard}>
          ...
          {editingProfile ? (
            <>
              ...
              <TextInput style={styles.input} placeholder="Tutor Display Name (optional)" placeholderTextColor="rgba(255,255,255,0.6)" value={tutorDisplayName} onChangeText={setTutorDisplayName} />
            ...
            <>
              ...
              <View style={styles.infoRow}><Ionicons name="ribbon-outline" size={16} color="rgba(255,255,255,0.85)" /><Text style={styles.infoText}>{user.tutorDisplayName || "Tutor name not set"}</Text></View>
            </>
          )}
        </BlurView>

        <TouchableOpacity activeOpacity={0.85} onPress={() => navigation.navigate("Settings")} style={styles.settingsBtn}>
        ...
        {isAdmin && (
          <TouchableOpacity activeOpacity={0.85} onPress={() => navigation.navigate("AdminVetting")}>
            <BlurView intensity={28} tint="light" style={styles.actionCard}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.actionTitle}>Admin: Vetting Questions</Text>
                <Text style={styles.actionSub}>Manage tutor application questions</Text>
              </View>
              <Ionicons name="chevron-forward" size={20} color="rgba(255,255,255,0.8)" />
            </BlurView>
          </TouchableOpacity>
          ...
          <>
            ...
            {apps.map((a) => (
              <TouchableOpacity 
                key={a.applicationId} 
                activeOpacity={0.85} 
                onPress={() => a.status === "PENDING_TEST" ? navigation.navigate("VettingTest", { appId: a.applicationId }) : Alert.alert("Application Status", "Your application is currently: " + a.status.replace("_", " "))}
              >
                <BlurView intensity={24} tint="light" style={styles.appCard}>
                  <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  settingsBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 12, paddingVertical: 14, borderRadius: 12, backgroundColor: "rgba(255,255,255,0.1)", borderWidth: 1, borderColor: "rgba(255,255,255,0.2)" },
  settingsBtnText: { color: "#fff", fontWeight: "600", fontSize: 15 },
  ...
  deleteBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 8, paddingVertical: 9, borderRadius: 10, backgroundColor: "rgba(255,0,0,0.1)", borderWidth: 1, borderColor: "rgba(255,0,0,0.3)" },
