...
export default function TutorsScreen() {
  ...
  const handleSearch = async () => {
    ...
    try { 
      ...
      const filtered = (Array.isArray(data) ? data : []).filter(tutorId => tutorId !== meId);
      setTutors(filtered);
  ...
  const handleBook = async (tutorId) => {
    ...
    try {
      ...
      Alert.alert("Booking confirmed", `${hours} hr with ${displayName}\nTotal: ${b.grossAmount} ${b.currency}\nStatus: ${b.status}`);
                ...
                <BlurView key={tutorId} intensity={26} tint="light" style={styles.tutorCard}>
                  <View style={styles.tutorHead}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.tutorCourse}>{course.trim().toUpperCase()} {email ? `| ${email}` : ""}</Text>
                    </View>
                  </View>
