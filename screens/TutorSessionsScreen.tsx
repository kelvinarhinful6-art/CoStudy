...
export default function TutorSessionsScreen({ navigation }: StackProps<"TutorSessions">) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setLinks(initial);
      // Resolve student names so the tutor can see who booked each session.
      listUsers()
        .then((users) => {
          ...
          sessions.map((s) => {
            ...
            <BlurView key={s.bookingId} intensity={26} tint="light" style={styles.card}>
              <View style={styles.cardTop}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.course}>{studentName || s.courseId}</Text>
                  <Text style={styles.meta}>{s.courseId ? s.courseId + " · " : ""}{s.hours} hr · {s.grossAmount} {s.currency} · {s.status}</Text>
                </View>
              </View>
