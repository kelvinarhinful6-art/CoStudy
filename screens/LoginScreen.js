...
export default function LoginScreen({ navigation }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      else { await login((email || username).trim(), password); }
      navigation.replace("Main");
    } catch (e) {
      Alert.alert(isRegister ? "Sign up failed" : "Sign in failed", e.message);
        ...
        <BlurView intensity={30} tint="light" style={styles.card}>
          ...
          {isRegister && (
            <View style={styles.inputRow}>
              ...
              <TextInput style={styles.input} placeholder="Username" placeholderTextColor="rgba(255,255,255,0.7)" value={username} onChangeText={setUsername} autoCapitalize="none" />
            </View>
          )}
          <View style={styles.inputRow}>
            <Ionicons name="mail-outline" size={20} color="rgba(255,255,255,0.9)" />
            <TextInput style={styles.input} placeholder={isRegister ? "Email" : "Email or username"} placeholderTextColor="rgba(255,255,255,0.7)" value={email} onChangeText={setEmail} autoCapitalize="none" />
          </View>
          <View style={styles.inputRow}>
          ...
          <TouchableOpacity style={styles.button} activeOpacity={0.85} onPress={handleSubmit} disabled={loading}>
            {loading ? <ActivityIndicator color="#0b6f8e" /> : <Text style={styles.buttonText}>{isRegister ? "Sign up" : "Sign in"}</Text>}
          </TouchableOpacity>
        </BlurView>
        <TouchableOpacity onPress={() => setMode(isRegister ? "login" : "register")}>
          <Text style={styles.footer}>{isRegister ? "Already have an account? " : "New here? "}<Text style={styles.footerLink}>{isRegister ? "Sign in" : "Create account"}</Text></Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 22, padding: 22, overflow: "hidden", borderWidth: 1, borderColor: "rgba(255,255,255,0.25)" },
  ...
  inputRow: { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "rgba(255,255,255,0.16)", borderWidth: 1, borderColor: "rgba(255,255,255,0.25)", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, marginBottom: 12 },
  input: { flex: 1, fontSize: 15, color: "#ffffff" },
  button: { backgroundColor: "#ffffff", borderRadius: 12, paddingVertical: 14, alignItems: "center", marginTop: 6, minHeight: 48, justifyContent: "center" },
