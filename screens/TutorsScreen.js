...
import { proStatus, proPlan, subscribePro, listTutors, bookTutor } from "../api";
...
export default function TutorsScreen() {
  ...
  const [hoursByTutor, setHoursByTutor] = useState({});
  const [bookingId, setBookingId] = useState(null);
  const [refreshing, setRefreshing] = useState(false);

  const loadStatus = useCallback(async () => {
    try {
      const [s, p] = await Promise.all([proStatus(), proPlan()]);
      setIsPro(!!s.active); setPlan(p);
    } catch (e) {} finally { setChecking(false); setRefreshing(false); }
  }, []);
  ...
  const handleSubscribe = async () => {
    setSubscribing(true);
    try { await subscribePro(); await loadStatus(); Alert.alert("You are Pro", "Tutor access unlocked."); }
    catch (e) { Alert.alert("Upgrade failed", e.message); }
    finally { setSubscribing(false); }
  };

  const handleSearch = async () => {
    if (!course.trim()) { Alert.alert("Enter a course", "Type a course code like PHY101."); return; }
    setSearching(true); setSearched(true);
    try { const data = await listTutors(course.trim().toUpperCase()); setTutors(Array.isArray(data) ? data : []); }
    catch (e) { setTutors([]); Alert.alert("Could not load tutors", e.message); }
    finally { setSearching(false); }
  };

  const setHours = (tutorId, h) => setHoursByTutor((m) => ({ ...m, [tutorId]: h }));
  ...
  const handleBook = async (tutorId) => {
    ...
    try {
      ...
      Alert.alert("Booking confirmed", `${hours} hr with ${tutorId}\nTotal: ${b.grossAmount} ${b.currency}\nStatus: ${b.status}`);
    } catch (e) { Alert.alert("Booking failed", e.message); }
    finally { setBookingId(null); }
  };

  if (checking) {
    return (<SkyBackground><View style={styles.loaderWrap}><ActivityIndicator color="#fff" /></View></SkyBackground>);
  }
      ...
      >
        ...
        {!isPro ? (
          <BlurView intensity={30} tint="light" style={styles.paywall}>
            ...
            <Text style={styles.paywallSub}>Unlock unlimited tutor access, priority matching and an ad-free experience.</Text>
            {plan && (<Text style={styles.price}>{plan.price} {plan.currency} / {plan.months === 1 ? "month" : plan.months + " months"}</Text>)}
            ...
            {tutors.map((tutorId) => (
              <BlurView key={tutorId} intensity={26} tint="light" style={styles.tutorCard}>
                <View style={styles.tutorHead}>
                  <View style={styles.avatar}><Ionicons name="person" size={20} color="#0b6f8e" /></View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.tutorName}>{tutorId}</Text>
                    <Text style={styles.tutorCourse}>{course.trim().toUpperCase()}</Text>
                  </View>
                </View>
                ...
                <View style={styles.hoursRow}>
                  {HOUR_OPTIONS.map((h) => {
                    ...
                    return (
                      <TouchableOpacity key={h} style={[styles.hourChip, sel && styles.hourChipSel]} onPress={() => setHours(tutorId, h)}>
                        <Text style={[styles.hourText, sel && styles.hourTextSel]}>{h} hr</Text>
                      </TouchableOpacity>
                    );
                  })}
                </View>
                <TouchableOpacity style={styles.bookBtn} onPress={() => handleBook(tutorId)} disabled={bookingId === tutorId} activeOpacity={0.85}>
                  {bookingId === tutorId ? <ActivityIndicator color="#fff" /> : <Text style={styles.bookText}>Book session</Text>}
                </TouchableOpacity>
              </BlurView>
            ))}
          </>
...
const styles = StyleSheet.create({
  ...
  paywall: { borderRadius: 20, padding: 24, alignItems: "center", gap: 10, overflow: "hidden", borderWidth: 1, borderColor: "rgba(255,255,255,0.25)" },
  ...
  button: { backgroundColor: "#fff", borderRadius: 12, paddingVertical: 13, paddingHorizontal: 24, alignItems: "center", justifyContent: "center", minHeight: 46, marginTop: 8, alignSelf: "stretch" },
  buttonText: { color: "#0b6f8e", fontWeight: "600", fontSize: 15 },
  proBadge: { flexDirection: "row", alignItems: "center", gap: 4, alignSelf: "flex-start", backgroundColor: "#fff", borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4, marginBottom: 14 },
  proBadgeText: { color: "#0b6f8e", fontWeight: "700", fontSize: 12 },
  searchCard: { borderRadius: 18, padding: 16, overflow: "hidden", marginBottom: 20, borderWidth: 1, borderColor: "rgba(255,255,255,0.22)" },
  ...
  tutorCard: { borderRadius: 18, padding: 16, overflow: "hidden", marginBottom: 14, borderWidth: 1, borderColor: "rgba(255,255,255,0.2)" },
  ...
  hourChip: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: "rgba(255,255,255,0.16)", borderWidth: 1, borderColor: "rgba(255,255,255,0.25)" },
  hourChipSel: { backgroundColor: "#fff" },
  hourText: { color: "#fff", fontWeight: "600", fontSize: 13 },
  hourTextSel: { color: "#0b6f8e" },
  bookBtn: { backgroundColor: "rgba(255,255,255,0.18)", borderWidth: 1, borderColor: "rgba(255,255,255,0.4)", borderRadius: 12, paddingVertical: 13, alignItems: "center", justifyContent: "center", minHeight: 46 },
