...
import { session, groupMessages, getBooking, WS_URL } from "../api";
...
export default function SessionChatScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    groupMessages(channel).then((d) => { if (active) setMessages(Array.isArray(d) ? d : []); }).catch(() => {}).finally(() => { if (active) setLoading(false); });
    // load the zoom link from the booking
    getBooking(bookingId).then((b) => { if (active && b) setZoomLink(b.zoomLink); }).catch(() => {});

    const client = new Client({
      brokerURL: WS_URL, reconnectDelay: 3000, forceBinaryWSFrames: true, appendMissingNULLonIncoming: true,
      onConnect: () => {
        setConnected(true);
        client.subscribe("/topic/group." + channel, (frame) => { try { addMessage(JSON.parse(frame.body)); } catch (e) {} });
      },
  ...
  const openZoom = async () => {
    ...
    try {
      ...
      if (ok) Linking.openURL(zoomLink); else Alert.alert("Cannot open link", zoomLink);
    } catch (e) { Alert.alert("Cannot open link", zoomLink); }
  };
    ...
    <SkyBackground>
      <View style={{ flex: 1, paddingTop: insets.top + 8 }}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.title}>{title || "Session"}</Text>
        ...
        {zoomLink ? (
          <TouchableOpacity style={styles.pinned} onPress={openZoom} activeOpacity={0.85}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.pinnedLink} numberOfLines={1}>{zoomLink}</Text>
            </View>
            <Text style={styles.joinText}>Join</Text>
          </TouchableOpacity>
        ) : (
          <View style={styles.noPin}><Ionicons name="videocam-outline" size={15} color="rgba(255,255,255,0.7)" /><Text style={styles.noPinText}>No Zoom link posted yet</Text></View>
        )}
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} keyboardVerticalOffset={8}>
          <View style={[styles.inputBar, { paddingBottom: insets.bottom + 8 }]}>
            <TextInput style={styles.input} placeholder="Message" placeholderTextColor="rgba(255,255,255,0.6)" value={text} onChangeText={setText} multiline />
            <TouchableOpacity style={styles.sendBtn} onPress={send} activeOpacity={0.85}><Ionicons name="send" size={18} color="#0b6f8e" /></TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  pinned: { flexDirection: "row", alignItems: "center", gap: 10, marginHorizontal: 14, marginBottom: 8, backgroundColor: "#fff", borderRadius: 14, paddingHorizontal: 14, paddingVertical: 11 },
  ...
  noPin: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, marginHorizontal: 14, marginBottom: 8, paddingVertical: 8, borderRadius: 12, backgroundColor: "rgba(255,255,255,0.1)", borderWidth: 1, borderColor: "rgba(255,255,255,0.2)" },
  ...
  inputBar: { flexDirection: "row", alignItems: "flex-end", gap: 8, paddingHorizontal: 12, paddingTop: 8, backgroundColor: "rgba(0,0,0,0.15)" },
